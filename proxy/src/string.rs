...
use arrow::{
    array::{ArrayDataBuilder, StringArray},
...
impl<K: AsPrimitive<usize> + FromPrimitive + Zero> PackedStringArray<K> {
    ...
    pub fn extend_from(&mut self, other: &PackedStringArray<K>) {
        ...
        // Copy offsets skipping the first element as this string start delimiter is
        // already provided by the end delimiter of the current offsets array
        self.offsets.extend(
            other
                .offsets
                .iter()
